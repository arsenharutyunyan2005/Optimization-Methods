...
class Segment:
    ...
    def calc(cls, a, b, eps, beta):
        ...
        while not cls.is_done(a, b, eps):
            ...
            print(f"  x1 = (a+b)/2 - β = {x1:.6f}   →   f(x1) = {fx1:.6f}")
            print(f"  x2 = (a+b)/2 + β = {x2:.6f}   →   f(x2) = {fx2:.6f}")
            ...
            if fx1 > fx2:
                a = x1
                print(f"  f(x1) > f(x2)  →  new a = x1 = {a}")
            else:
                b = x2
                print(f"  f(x1) ≤ f(x2)  →  new b = x2 = {b}")
        ...
        cls.plot_results(orig_a, orig_b, hist_x, hist_y, hist_iter,
                         x_star, f_star, a, b, eps, beta, i)

    @classmethod
    def plot_results(cls, orig_a, orig_b, hist_x, hist_y, hist_iter,
                     x_star, f_star, a, b, eps, beta, iterations):
        ...
        # ── Gradient curve ───────────────────────────────────────────
        x_smooth = np.linspace(orig_a + 0.3, orig_b - 0.2, 500)
        y_smooth = np.array([cls.new_function(v) for v in x_smooth])

        y_lo = y_smooth.min() - 0.3
        y_hi = max(cls.new_function(orig_a), cls.new_function(orig_b)) * 0.42
        ...
        for bound in [a, b]:
            ax.axvline(bound, color=ACCENT, lw=0.8, ls=":", alpha=0.5, zorder=4)

        # ── Optimum line ─────────────────────────────────────────────
        ax.axvline(x_star, color=ACCENT, lw=1.6, ls="--", alpha=0.9, zorder=5,
                   path_effects=[pe.withStroke(linewidth=4, foreground=BG)])

        ax.scatter([x_star], [f_star], marker="o", s=60,
                   color=ACCENT, edgecolors=TEXT, linewidths=1.0, zorder=8)
        ...
        # ── Title + subtitle ──────────────────────────────────────────
        ax.set_title("SEGMENTATION METHOD",
                     fontsize=14, fontweight="bold", color=GOLD1, pad=28,
                     path_effects=[pe.withStroke(linewidth=3, foreground=BG)])
        ax.text(0.5, 1.018,
                f"f(x) = eˣ - 4x + x²   |   ε = {eps}   |   β = {beta}   |   {iterations} iterations",
